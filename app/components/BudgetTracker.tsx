...
import { DollarSign, TrendingUp, ShoppingBag, Star, MapPin, Clock, Tag, CalendarDays, Calendar, CalendarRange, CalendarCheck, Users } from 'lucide-react'
...
export default function BudgetTracker() {
  ...
  const periods = [
    ...
    { id: 'quarter', label: 'This Quarter', icon: CalendarRange },
            ...
            <div key={index} className="flex items-center justify-between">
              ...
              <div className="flex items-center space-x-4">
                <div className="w-32 bg-gray-200 rounded-full h-2">
                  <div 
                    className="bg-warm-400 h-2 rounded-full transition-base" 
      ...
      <div className="card-warm">
        ...
        <div className="space-y-4">
          {recentPurchases.map((purchase) => (
            <div key={purchase.id} className="flex items-center justify-between bg-white rounded-xl shadow-md p-4 mb-2">
              ...
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-purple-500">{purchase.store}</p>
                </div>
              </div>
              {/* Right: Price, savings, rating */}
              <div className="text-right">
                <p className="font-bold text-gray-900">${purchase.price}</p>
                <p className="text-sm text-purple-500">Saved ${purchase.savings}</p>
                <div className="flex items-center space-x-1 justify-end">
                  <Star className="w-4 h-4 text-yellow-400 fill-current" />
                  <span className="text-sm text-purple-700">{purchase.rating}</span>
                </div>
              </div>
            </div>
          ))}
